locals {
  ...
  tag_policies = {
    for key, value in local.mandated_tags :
      replace(key, " ", "") => {  
        tags = {
            replace(key, " ", "") = {
              tag_key = { "@@assign" =  replace(key, " ", "") }
              tag_value = { "@@assign" = [value] }
...
resource "aws_organizations_policy" "tag_enforcement_policy" {
  ...
  #name    = "tag-policy-mandatory-tags-enforcement-${data.aws_ssm_parameter.app_code.value}-${local.mandatory_tags.Environment}-${each.key}"
